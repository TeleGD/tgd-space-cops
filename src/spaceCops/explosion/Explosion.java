...
public class Explosion extends Movable {
	...
	public void loadExplosion(){
		for(int i = 0; i<60; i++){
			if(i<8){
				try {
					image[i] = new Image("images/explosion/000"+(i+2)+".png");
				...
				try {
					image[i] = new Image("images/explosion/00"+(i+2)+".png");
				} catch (SlickException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
	...
	public void bruit() {
		try {
			Sound sound= new Sound("sounds/explosion.ogg");
			sound.play();
		} catch (SlickException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
