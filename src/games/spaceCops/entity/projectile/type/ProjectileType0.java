package games.spaceCops.entity.projectile.type;
...
// L'angle est en degres, et est compte en sens horaire.
	...
	public ProjectileType0(World world, double x, double y, double angle, double speed, double d, double n, boolean allied) {
	...
	// et qu'on est alles assez loin
		if((!forked)&&(d>maxDistance)){
			for(int i = 0; i<n; i++){
				children.add(new Projectile(world, x,y,angle+45-i*(90/(n-1)),Math.sqrt(Math.pow(speedY, 2)+Math.pow(speedX, 2)), alliedShot));
			}
			forked = true;
		}
	}
